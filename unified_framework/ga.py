# -*- coding: utf-8 -*-
"""
Genetic Algorithm (GA) solver for task-path co-optimization.
...
Encoding
--------
A chromosome is a continuous random-key vector with four dimensions per real
task: selection, day, location, and within-day order. The solver evolves these
random keys with tournament selection, arithmetic crossover, mutation, elitism,
and occasional immigrants, then decodes them into a complete 3-day route.
A repair/validation layer enforces the same structural constraints as ea.py:
required tasks, remote exactly-one, continuous exact-cardinality, day windows,
tag boundary constraints, time/power budgets, and per-step return safety.
...
class task_optimize(object):
    """GA-based scheduler with the same interface as the Gurobi version."""
    ...
    def __init__(
        ...
        dataFrames=None,
        writeOutput=True,
        # GA hyper-parameters (extra, ignored by ea.py callers)
        population_size=100,
        ...
        self.__mutation_scale = mutation_scale
        self.__elite_fraction = elite_fraction
        self.__immigrant_fraction = immigrant_fraction
    ...
    def gen_edges(self):
        pass  # Not needed for GA - edges are implicit in the route

    # -----------------------------------------------------------------------
    # Pre-compute lookup tables for fast evaluation inside GA
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # A solution (decoded chromosome) is a dict:
    ...
    # -----------------------------------------------------------------------
    # Solution operators (discrete / combinatorial)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # Continuous random-key decoding
    # -----------------------------------------------------------------------
    def _sigmoid(self, x):
        x = np.clip(x, -60.0, 60.0)
        return 1.0 / (1.0 + np.exp(-x))

    def _init_ga_space(self):
        self.__ga_tasks = list(self.__task_indices)
        self.__ga_index = {t: i for i, t in enumerate(self.__ga_tasks)}
    ...
    def _new_random_key_position(self):
        n = len(self.__ga_tasks)
        pos = {
    ...
    def _copy_random_key_position(self, pos):
    ...
    def _choose_by_key(self, candidates, key_array, reverse=True):
        ...
        return sorted(
            candidates,
            key=lambda t: key_array[self.__ga_index[t]],
            reverse=reverse,
        )
    ...
    def _decode_position(self, pos):
        ...
        for t in self.__ga_tasks:
            if t in selected:
                continue
            if select_score[self.__ga_index[t]] >= 0.5:
                selected.add(t)
        ...
        for t in list(selected):
            ...
            if not locs:
                selected.discard(t)
                continue
            raw = loc_score[self.__ga_index[t]]
        ...
        for t in selected:
            ...
            raw = day_score[self.__ga_index[t]]
            d = valid[min(int(raw * len(valid)), len(valid) - 1)]
            days[d].append(t)
        for d in range(3):
            days[d].sort(key=lambda t: pos["order"][self.__ga_index[t]])
    ...
    def _repair_decoded_solution(self, sol, pos=None):
        ...
        if rem_real:
            ...
            if len(required_rem) <= 1:
                if required_rem:
                    ...
                    key = pos["select"] if pos is not None else np.zeros(len(self.__ga_tasks))
        ...
        self._sort_days_by_position_order(sol, pos)
        self._repair_time_power(sol)
    ...
    def _sort_days_by_position_order(self, sol, pos=None):
        for d in range(3):
            if pos is not None:
                sol["days"][d].sort(key=lambda t: pos["order"][self.__ga_index[t]])
    ...
    def _repair_boundary_tags(self, sol, pos=None):
        ...
        def choose_pair_day(starts, ends, allowed):
            ...
            if pos is None:
                return allowed[0]
            vals = [self._sigmoid(pos["day"][self.__ga_index[t]]) for t in tasks]
        ...
        if d12 == 1 and d23 == 1:
            # Move the 23 pair to day 3 unless the keys strongly prefer moving 12 to day 1.
            d23 = 2
    ...
    def _copy_chromosome(self, chrom):
        return {k: np.array(v, copy=True) for k, v in chrom.items()}

    def _tournament_select(self, fitness):
        k = min(max(1, int(self.__tournament_size)), len(fitness))
        candidates = self.__rng.choice(len(fitness), size=k, replace=False)
        return int(max(candidates, key=lambda i: fitness[i]))

    def _crossover_position(self, parent_a, parent_b):
        if self.__rng.random() >= self.__crossover_rate:
            parent = parent_a if self.__rng.random() < 0.5 else parent_b
            return self._copy_chromosome(parent)

        child = {}
        for key in parent_a:
            mask = self.__rng.random(len(parent_a[key])) < 0.5
            uniform = np.where(mask, parent_a[key], parent_b[key])
            if self.__rng.random() < 0.35:
                alpha = self.__rng.random(len(parent_a[key]))
                blend = alpha * parent_a[key] + (1.0 - alpha) * parent_b[key]
                child[key] = 0.5 * uniform + 0.5 * blend
            else:
                child[key] = uniform
        return child

    def _mutate_position(self, chrom):
        child = self._copy_chromosome(chrom)
        for key in child:
            n = len(child[key])
            mask = self.__rng.random(n) < self.__mutation_rate
            if np.any(mask):
                child[key][mask] += self.__rng.normal(
                    0.0, self.__mutation_scale, int(np.sum(mask))
                )

            reset_mask = self.__rng.random(n) < (self.__mutation_rate * 0.12)
            if np.any(reset_mask):
                child[key][reset_mask] = self.__rng.normal(
                    0.0, 1.0, int(np.sum(reset_mask))
                )
        return child
    ...
    # -----------------------------------------------------------------------
    def run_opt(self):
        """Genetic algorithm loop over continuous random-key chromosomes."""
        import time as _time

        self._init_ga_space()
        n_pop = max(2, int(self.__population_size))
        max_generations = max(1, int(self.__max_generations))
        start_time = _time.time()
        time_limit = self.__timeLimit if self.__timeLimit != np.inf else 1e18

        population = [self._new_chromosome() for _ in range(n_pop)]
        fitness, sols = [], []
        for chromosome in population:
            fit, sol = self._fitness_from_position(chromosome)
            fitness.append(fit)
            sols.append(sol)
        ...
        print(
            f"[GA] Initial fitness={gbest_fit:.4f}  "
            f"rev={init_rev:.2f}  feasible={self._is_feasible(gbest_sol)}"
        )

        stagnation = 0
        no_improve_limit = max(150, max_generations // 4)
        elite_count = max(1, min(n_pop, int(np.ceil(n_pop * self.__elite_fraction))))
        immigrant_count = max(
            0,
            min(n_pop - elite_count, int(round(n_pop * self.__immigrant_fraction))),
        )

        for gen in range(max_generations):
            if _time.time() - start_time >= time_limit:
                print("[GA] Time limit reached.")
                break

            sorted_idx = np.argsort(fitness)[::-1]
            new_population = [
                self._copy_chromosome(population[i])
                for i in sorted_idx[:elite_count]
            ]

            target_children = n_pop - immigrant_count
            while len(new_population) < target_children:
                p1 = population[self._tournament_select(fitness)]
                p2 = population[self._tournament_select(fitness)]
                child = self._crossover_position(p1, p2)
                child = self._mutate_position(child)
                new_population.append(child)

            while len(new_population) < n_pop:
                new_population.append(self._new_chromosome())
            ...
            if fitness[gen_best_idx] > gbest_fit:
                gbest_fit = fitness[gen_best_idx]
                gbest_sol = copy.deepcopy(sols[gen_best_idx])
                stagnation = 0
                self._save_incumbent(gbest_sol)
            else:
                stagnation += 1

            if gen > 0 and gen % 50 == 0 and self._is_feasible(gbest_sol):
            ...
            if stagnation >= no_improve_limit:
                sorted_idx = np.argsort(fitness)[::-1]
                keep_count = max(1, n_pop // 5)
                population = [
                    self._copy_chromosome(population[i])
                    for i in sorted_idx[:keep_count]
                ]
                while len(population) < n_pop:
                    population.append(self._new_chromosome())
                fitness, sols = [], []
                for chromosome in population:
                    fit, sol = self._fitness_from_position(chromosome)
                    fitness.append(fit)
        ...
        print(
            f"[GA] Best fitness={gbest_fit:.4f}  "
            f"rev={best_rev:.2f}  "
            f"time={best_time:.2f}  "
    ...
    # -----------------------------------------------------------------------
    # Convert GA solution to ea.py result format
    # -----------------------------------------------------------------------
    ...
    def print_status(self):
        ...
        if best is None or not self._is_feasible(best):
            raise MIPError("GA did not find a feasible solution")

        print(f"GA feasible incumbent objective value: {self.__objvalue}")
...
def solve(case, mode="normal"):
    """Run the GA algorithm on UnifiedCase input."""
    if mode != "normal":
        raise NotImplementedError("ga only supports algorithm.mode=normal")
