...
def _as_expr(value):
    if isinstance(value, _LinearExpr):
        return value
    if isinstance(value, _HighsVar):
        return _LinearExpr({value.index: 1.0})
    if isinstance(value, (int, float, np.integer, np.floating)):
        return _LinearExpr({}, float(value))
...
class _LinearExpr:
    ...
    def __add__(self, other):
        ...
        for index, coeff in other.terms.items():
            terms[index] = terms.get(index, 0.0) + coeff
            if abs(terms[index]) <= 1e-12:
                del terms[index]
        return _LinearExpr(terms, self.constant + other.constant)

    __radd__ = __add__

    def __neg__(self):
        return _LinearExpr(
            {index: -coeff for index, coeff in self.terms.items()},
            -self.constant,
        )
    ...
    def __mul__(self, other):
        ...
        other = float(other)
        return _LinearExpr(
            {index: coeff * other for index, coeff in self.terms.items()},
            self.constant * other,
        )
    ...
    def __le__(self, other):
        ...
        if other is NotImplemented:
            return NotImplemented
        return _LinearConstr(self - other, -highspy.kHighsInf, 0.0)

    def __ge__(self, other):
        other = _as_expr(other)
        if other is NotImplemented:
            return NotImplemented
        return _LinearConstr(self - other, 0.0, highspy.kHighsInf)

    def __eq__(self, other):
        other = _as_expr(other)
        if other is NotImplemented:
            return NotImplemented
        return _LinearConstr(self - other, 0.0, 0.0)
...
def _resolve_parallel_workers(requested):
    requested = (
        ...
        else os.environ.get("TOP_EAH_PARALLEL_WORKERS")
        or os.environ.get("TOP_EAH_WORKERS")
        or os.environ.get("TOP_EAO_PARALLEL_WORKERS")
...
def _resolve_parallel_edge_threshold():
    return _coerce_positive_int(
        os.environ.get("TOP_EAH_PARALLEL_EDGE_THRESHOLD")
        or os.environ.get("TOP_EAO_PARALLEL_EDGE_THRESHOLD"),
        20000,
    )
...
class _HighsParams:
    ...
    def __setattr__(self, name, value):
        ...
        if name == "TimeLimit":
            model.setOptionValue("time_limit", float(value))
        elif name == "SolutionLimit":
            try:
                model.setOptionValue("solution_limit", int(value))
            except Exception:
                return
        elif name == "Threads":
            ...
            try:
                ...
                if threads > 1:
                    ...
                    model.setOptionValue("mip_min_cliquetable_entries_for_parallelism", 1)
                else:
                    model.setOptionValue("parallel", "choose")
            except Exception:
                pass
        elif name == "OutputFlag":
            enabled = _coerce_bool(value, False)
            model.setOptionValue("output_flag", enabled)
            model.setOptionValue("log_to_console", enabled)
        elif name == "MipLogInterval":
            model.setOptionValue("mip_min_logging_interval", float(value))
        elif name in {"MIPFocus", "Heuristics"}:
            # Gurobi-specific tuning parameters have no direct HiGHS equivalent here.
            return
        else:
...
class _HighsModel:
    def __init__(self, name):
        ...
        try:
            self._model.HandleKeyboardInterrupt = True
            self._model.HandleUserInterrupt = True
        except Exception:
            pass
        self._model.setOptionValue("output_flag", False)
        self.Params = _HighsParams(self._model)
        self.Status = None
        self.SolCount = 0
        self.objVal = None
    ...
    def addVars(self, keys, vtype="C", name="", ub=None):
        ...
        if not key_list:
            return variables
        created_vars = self._add_vars(len(key_list), vtype, ub=ub)
        for key_tuple, var in zip(key_list, created_vars):
            variables.add(key_tuple, var)
        return variables

    def _add_vars(self, count, vtype, ub=None):
        start_index = self._var_count
        lower = np.zeros(count, dtype=np.double)
        if vtype == _ScipGRB.BINARY:
            upper_value = 1.0
        elif ub is None:
            upper_value = highspy.kHighsInf
        else:
            upper_value = float(ub)
        upper = np.full(count, upper_value, dtype=np.double)
        status = self._model.addVars(count, lower, upper)
        if status != highspy.HighsStatus.kOk:
            raise MIPError(f"HiGHS failed to add {count} variables: {status}")
        if vtype == _ScipGRB.BINARY:
            indexes = np.arange(start_index, start_index + count, dtype=np.int32)
            integrality = np.full(
                count,
                int(highspy.HighsVarType.kInteger),
                dtype=np.uint8,
            )
            status = self._model.changeColsIntegrality(count, indexes, integrality)
            if status != highspy.HighsStatus.kOk:
                raise MIPError(f"HiGHS failed to mark binary variables: {status}")
        self._var_count += count
        return [_HighsVar(index) for index in range(start_index, start_index + count)]

    def _add_named_vars(self, keys, vtype="C", name=""):
        key_list = [tuple(key) for key in keys]
        arity = len(key_list[0]) if key_list else 0
        variables = _VarDict(arity)
        for key_tuple in key_list:
            suffix = ",".join(str(part) for part in key_tuple)
            variables.add(key_tuple, self._add_var(vtype, f"{name}[{suffix}]" if name else ""))
        return variables

    def _add_var(self, vtype, name):
        index = self._var_count
        upper = 1.0 if vtype == _ScipGRB.BINARY else highspy.kHighsInf
        self._model.addVar(0.0, upper)
        if vtype == _ScipGRB.BINARY:
            self._model.changeColIntegrality(index, highspy.HighsVarType.kInteger)
        if name:
            try:
                self._model.passColName(index, name)
            except Exception:
                pass
        self._var_count += 1
        return _HighsVar(index)
    ...
    def addConstr(self, cons, name=""):
        ...
        return self._add_linear_constraint(cons, name)
    ...
    def addConstrs(self, constrs, name=""):
        ...
        return self._add_linear_constraints(constraints)

    def _add_linear_constraint(self, cons, name):
        if not isinstance(cons, _LinearConstr):
            raise TypeError(f"Unsupported HiGHS constraint type: {type(cons).__name__}")
        items = [(index, coeff) for index, coeff in cons.expr.terms.items() if abs(coeff) > 1e-12]
        indexes = np.array([index for index, _ in items], dtype=np.int32)
        values = np.array([coeff for _, coeff in items], dtype=np.double)
        lower = cons.lower - cons.expr.constant
        upper = cons.upper - cons.expr.constant
        row_index = self._row_count
        self._model.addRow(lower, upper, len(items), indexes, values)
        if name:
            try:
                self._model.passRowName(row_index, name)
            except Exception:
                pass
        self._row_count += 1
        return row_index

    def _add_linear_constraints(self, constraints):
        if not constraints:
            return []
        lower = np.empty(len(constraints), dtype=np.double)
        upper = np.empty(len(constraints), dtype=np.double)
        starts = np.empty(len(constraints), dtype=np.int32)
        indexes = []
        values = []
        nnz = 0
        for row_index, cons in enumerate(constraints):
            if not isinstance(cons, _LinearConstr):
                raise TypeError(f"Unsupported HiGHS constraint type: {type(cons).__name__}")
            starts[row_index] = nnz
            lower[row_index] = cons.lower - cons.expr.constant
            upper[row_index] = cons.upper - cons.expr.constant
            for index, coeff in cons.expr.terms.items():
                if abs(coeff) <= 1e-12:
                    continue
                indexes.append(index)
                values.append(coeff)
                nnz += 1
        index_array = np.array(indexes, dtype=np.int32)
        value_array = np.array(values, dtype=np.double)
        status = self._model.addRows(
            len(constraints),
            lower,
            upper,
            nnz,
        ...
        if status != highspy.HighsStatus.kOk:
            raise MIPError(f"HiGHS failed to add {len(constraints)} constraints: {status}")
        first_row = self._row_count
        self._row_count += len(constraints)
        return list(range(first_row, self._row_count))

    def addConsIndicator(self, cons, binvar=None, activeone=True, name=""):
        raise NotImplementedError("HiGHS adapter does not support indicator constraints")

    def setObjective(self, expr, sense):
        expr = _as_expr(expr)
        if expr is NotImplemented:
            raise TypeError(f"Unsupported HiGHS objective type: {type(expr).__name__}")
        if expr.terms:
            indexes = np.array(list(expr.terms.keys()), dtype=np.int32)
            costs = np.array(list(expr.terms.values()), dtype=np.double)
            status = self._model.changeColsCost(len(indexes), indexes, costs)
            if status != highspy.HighsStatus.kOk:
                raise MIPError(f"HiGHS failed to set objective costs: {status}")
        if abs(expr.constant) > 1e-12:
            try:
                self._model.changeObjectiveOffset(expr.constant)
            except Exception:
                pass
        if sense == _ScipGRB.MAXIMIZE:
            self._model.changeObjectiveSense(highspy.ObjSense.kMaximize)
        return expr

    def setObjectiveCoeffs(self, coeffs, sense):
        if coeffs:
            indexes = np.array(list(coeffs.keys()), dtype=np.int32)
            costs = np.array(list(coeffs.values()), dtype=np.double)
            status = self._model.changeColsCost(len(indexes), indexes, costs)
            if status != highspy.HighsStatus.kOk:
                raise MIPError(f"HiGHS failed to set objective costs: {status}")
        if sense == _ScipGRB.MAXIMIZE:
    ...
    def _capture_status(self):
        ...
        status_text = self._model.modelStatusToString(status).lower()
        if "optimal" in status_text:
            self.Status = _ScipStatus.OPTIMAL
        elif "infeasible" in status_text:
            self.Status = _ScipStatus.INFEASIBLE
        elif self._interrupted or "interrupt" in status_text or "cancel" in status_text:
            self.Status = _ScipStatus.USER_INTERRUPT
        elif "limit" in status_text or "time" in status_text:
            self.Status = _ScipStatus.TIMELIMIT
        else:
            self.Status = status_text
        try:
            self._solution_values = list(self._model.getSolution().col_value)
        except Exception:
            self._solution_values = []
        try:
            obj_value = float(self._model.getObjectiveValue())
        except Exception:
            try:
                obj_value = float(self._model.getInfo().objective_function_value)
            except Exception:
                obj_value = None
        has_solution = (
            self._solution_values
            and self.Status != _ScipStatus.INFEASIBLE
            and obj_value is not None
            and np.isfinite(obj_value)
        )
        self.SolCount = 1 if has_solution else 0
        if has_solution:
            self.objVal = obj_value
        else:
            self.objVal = None

    def interruptSolve(self):
        self._interrupted = True
        return self._model.cancelSolve()
    ...
    def write(self, path):
        if path.endswith(".sol") and self.SolCount >= 1:
            try:
                return self._model.writeSolution(path, 0)
            except TypeError:
                return self._model.writeSolution(path)
        return self._model.writeModel(path)

    def getVal(self, var):
        if not self._solution_values:
            self._solution_values = list(self._model.getSolution().col_value)
        return self._solution_values[var.index]

    def getBestSol(self):
        return None

    def getSolVal(self, sol, var):
        return sol.get(var.index, 0.0)
    ...
    def setSolVal(self, sol, var, value):
        sol[var.index] = float(value)
        return None

    def trySol(self, sol, printreason=False):
        if not sol:
            return False
        items = sorted(sol.items())
        indexes = np.array([index for index, _ in items], dtype=np.int32)
        values = np.array([value for _, value in items], dtype=np.double)
        try:
            status = self._model.setSolution(len(items), indexes, values)
        except TypeError:
            highs_sol = highspy.HighsSolution()
            highs_sol.col_value = [0.0] * self._var_count
            for index, value in items:
                highs_sol.col_value[index] = value
            highs_sol.value_valid = True
            status = self._model.setSolution(highs_sol)
...
class task_optimize(object):
    def __init__(
        ...
        self.__highs_log = _coerce_bool(
            highsLog if highsLog is not None else os.environ.get("TOP_EAH_HIGHS_LOG"),
            True,
        )
        self.__m = gp.Model("schedule-optimization")
        self.__m.Params.OutputFlag = self.__highs_log
        if self.__highs_log:
            self.__m.Params.MipLogInterval = 1.0
    ...
    def __safe_big_m(self, resource_bound, return_cost, task_cost=0.0, eps=0.0):
        return exact_big_m(resource_bound, return_cost, task_cost, eps)

    def __transition_big_m(self, resource_bound, edge_cost):
        return exact_big_m(resource_bound, edge_cost)
    ...
    def gen_edges(self):
        ...
        if use_parallel:
            try:
                ...
                print(
                    "[eah] PARALLEL gen_edges failed; falling back to serial "
    ...
    def set_objective(self):
        ...
        if self.__objective == self.__Obj[0]:
            ...
            coeffs = {
                var.index: float(revenue[t])
                for (_, _, _, t), var in self.__x.items()
                if float(revenue[t]) != 0.0
            }
            self.__m.setObjectiveCoeffs(coeffs, gp.GRB.MAXIMIZE)
            return coeffs
        if self.__objective == self.__Obj[1]:
            coeffs = {self.__W[*self.__opoint[-1]].index: -1.0}
            self.__m.setObjectiveCoeffs(coeffs, gp.GRB.MAXIMIZE)
            return coeffs
        if self.__objective == self.__Obj[2]:
            coeffs = {self.__Q[*self.__opoint[-1]].index: -1.0}
            self.__m.setObjectiveCoeffs(coeffs, gp.GRB.MAXIMIZE)
            return coeffs
        self.__m.setObjectiveCoeffs({}, gp.GRB.MAXIMIZE)
        return {}
    ...
    def add_safe_constrs(self):
        ...
        for i, k in point:
            ...
            time_m = self.__safe_big_m(time_bound, safe_time_to_base[i], task_time[k], eps)
            power_m = self.__safe_big_m(power_bound, safe_power_from_base[i], task_power[k], eps)
    ...
    def _save_incumbent_solution(self):
        try:
            ...
            print(
                f"[eah] INCUMBENT sol={self.__solcount} time={self.__m.getSolvingTime():.3f}s "
            ...
            print(
                f"[eah] INCUMBENT autosave failed error={exc.__class__.__name__}: {exc}",
                flush=True,
            )
        return None
        ...
        for route in route_by_day.values():
            ...
            for source, target in zip(route, route[1:]):
                ...
                if edge not in self.__x:
                    raise ValueError(f"Warm-start edge is not in HiGHS graph: {edge}")
    ...
    def __print_solution_summary(self, label, schedule_df, obj_value=None, path=None):
        ...
        print(
            f"[eah] SOLUTION {label} obj={obj_text} "
    ...
    def __validate_business_schedule(self, schedule_df, label="schedule"):
        ...
        if violations:
            print(
                f"[eah] VALIDATE {label} failed day_time={day_time} day_power={day_power} "
                f"violations={';'.join(violations)}",
                flush=True,
            )
            return False
        print(
            f"[eah] VALIDATE {label} ok day_time={day_time} day_power={day_power}",
            flush=True,
        )
        return True
    ...
    def __try_warmstart_schedule(self, schedule_df, label, printreason, path=None):
        ...
        print(
            f"[eah] WARMSTART {label} accepted={accepted} "
    ...
    def build_heuristic_start(self):
        ...
        print("[eah] WARMSTART running ha for initial feasible route", flush=True)
        ...
        os.makedirs(autosave_dir, exist_ok=True)
        ha_path = os.path.join(autosave_dir, "eah_ha_warmstart.csv")
        ha_schedule.to_csv(ha_path, index=False)
        print(f"[eah] WARMSTART ha_schedule={ha_path}", flush=True)
    ...
    def run_opt(self):
        if self.__lppath != None:
            ...
            self.__m.update()
        self.__m.printStats()
        interrupted = {"value": False, "count": 0}
        solve_error = []
        ...
        def stop_scip(signum, frame):
            ...
            if interrupted["count"] >= 2:
                print(
                    "[eah] INTERRUPT second Ctrl+C received; forcing Python process exit.",
                    flush=True,
                )
                os._exit(130)
            print(
                "[eah] INTERRUPT Ctrl+C received; stopping HiGHS and keeping current incumbent... "
            ...
            try:
                ...
                print(
                    f"[eah] INTERRUPT failed to request HiGHS stop: {exc.__class__.__name__}: {exc}",
                    flush=True,
                )
        ...
        try:
            ...
            if install_sigint_handler:
                solve_thread = threading.Thread(
                    ...
                    name="eah-highs-optimize",
                    daemon=True,
                )
                solve_thread.start()
                while solve_thread.is_alive():
                    solve_thread.join(0.25)
        ...
        if interrupted["value"] or self.__m.Status == _ScipStatus.USER_INTERRUPT:
            ...
            print(
                f"[eah] INTERRUPT stopped status={self.__m.Status} "
    ...
    def print_status(self):
        if self.__m.Status == gp.GRB.OPTIMAL:
            ...
            print(
                "[eah] FALLBACK using HA schedule because HiGHS found no incumbent",
                flush=True,
            )
    ...
    def __run_logged_step(self, index, total, step, func):
        print(f"[eah] START {index:02d}/{total:02d} {step}", flush=True)
    ...
    def run(self):
        print(
            f"[eah] RUN start objective={self.__objective} timeLimit={self.__time_limit} "
            f"parallelWorkers={self.__parallel_workers} highsLog={self.__highs_log}",
            flush=True,
        )
...
def solve(case, mode="normal"):
    """Run the HiGHS exact algorithm on UnifiedCase input."""
    if mode != "normal":
        raise NotImplementedError("eah only supports algorithm.mode=normal")
    ...
    highs_log = (
        algorithm_raw.get("highsLog")
        if "highsLog" in algorithm_raw
        else algorithm_raw.get("highs_log")
        if "highs_log" in algorithm_raw
